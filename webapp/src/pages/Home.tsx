  ...
  return (
    ...
    <div className="card card-hot">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-xl font-bold text-white">🔥 Горячий прогноз</h2>
        <span className="text-xs bg-white/20 px-2 py-1 rounded-full text-white">LIVE</span>
      </div>
      {isLoading ? (
        ...
        <div>
          <p className="text-white/90 mb-3">Сегодня нет матчей с высокой уверенностью</p>
          <Link
            to="/prediction"
            className="block w-full bg-white text-orange-600 text-center py-2 rounded-xl font-semibold active:scale-95 transition-transform"
        ...
        <div className="space-y-3">
          <div className="text-lg font-semibold text-white">
            {hot.home_team} <span className="text-white/70">vs</span> {hot.away_team}
          </div>
          <div className="text-sm text-white/90">🏆 {hot.league_name}</div>
          <div className="text-3xl font-bold text-white">{hot.hot_confidence.toFixed(0)}%</div>
          <div className="text-sm text-white/90">💎 {hot.hot_bet}</div>
          <Link
            to={`/prediction?team1=${encodeURIComponent(hot.home_team)}&team2=${encodeURIComponent(hot.away_team)}&league=${hot.league}`}
            className="block w-full bg-white text-orange-600 text-center py-2 rounded-xl font-semibold mt-3 active:scale-95 transition-transform"
            onClick={() => hapticFeedback('medium')}
          >
            Подробнее →
          </Link>
